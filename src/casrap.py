...
class App(object):
    ...
    def load_platform(self, config):
        ...
        for plat_code, plat in config.items():
            ...
            plat["idhash"] = Hashids(salt=self.system["key"] + plat["code"], min_length=16, alphabet="abcdefghijklmnopqrstuvwxyz0123456789")
    ...
    def reload(self):
        ...
        self.resource = {}
        self.load_resource(config["resource"])
    ...
    def get_client(self, client_ip, uahash, ja3hash, ua, ja3):
        ...
        self.db.uac_clientinfo.append({"client_sn": client_sn, "client_ip": client_ip, "uahash": uahash, "ja3hash": ja3hash, "ua": ua, "ja3": ja3, "created_at": datetime.datetime.now()})
    ...
    def cmd__sysinfo(self, castag):
        ...
        r = {"{}$castag".format(syscode): str(self.castag)}
    ...
    def cmd__appinfo(self, platcode, url, session_code, client_sn, client_ip, ua, ja3, ja3hash, appcode=None):
        ...
        print(self.system["debug"], ">>>>>>>>>>>>>>>>>>>>>")
        print(platcode, appcode, app)
        ...
        ins_sn = plat["idhash"].encode((session_id or 0), (user_id or 0), self.machine_id, self.timer(), self.counter())
    ...
    def cmd__redis(self, *args):
        try:
            r = self.redis.execute_command(*args)
            print(r, type(r))

            try:
                return 200, {}, pickle.loads(r)
            except:
                pass
    ...
    def casrap__set_session(self, _, platcode, user_id, user_name, user_account, session_id):
        ...
        if plat["singleton"]:
            last_session_id = self.redis.get(f"LASTSESSION_{user_id}_{platcode}")

            if last_session_id:
                last_session_key = "ssn_{}_{}".format(platcode, last_session_id.decode())
                self.redis.hset(last_session_key, "expires", -1)

        self.redis.set(f"LASTSESSION_{user_id}_{platcode}", int(session_id))
    ...
    def casrap__get_roles(self, __session):
        return 200, {}, {"roles": [{"code": r["code"], "name": r["name"]} for r in self.roles.values()]}
    ...
    def casrap__test_acl(self, __session, user_id, platform_key, api):
        ...
        return 200, {}, {"userroles": list(userroles), "apiroles": list(apiroles), "authrole": apiroles & userroles, "authpass": bool(authrole)}
    ...
    def casrap__get_menu(self, __session, root, depth, appcode=None):
        ...
        return 200, {}, {"root": root, "depth": depth, "menus": rootm["children"] if rootm else []}
    ...
    def casrap__get_usermenu(self, __session, user_id=None):
        ...
        for menucode, menudata in self.menu.items():
            ...
            if set(menudata["allow"]) & {f"{__session['platcode']}:{r}" for r in roles}:
                r.append({"name": menudata["name"], "menu_type": menudata["menu_type"], "code": menudata["code"], "url": menudata["url"]})
    ...
    def casrap__get_platforms(self, __session):
        plats = [{"code": p["code"], "name": p["name"], "urlroot": p["urlroot"], "close": p["close"], "appclose": p["appclose"]} for p in self.platforms.values()]
    ...
    def casrap__close_platform(self, __session, platcode, close, appclose=None):
        ...
        return 200, {}, {"code": plat["code"], "name": plat["name"], "close": plat["close"], "appclose": plat["appclose"]}

    def casrap__get_resource(self, __session, name=None):
        user_id = __session["user_id"]

        if not user_id and __session["platcode"] in self.system["debug"]:
            res = {}
            if self.resource:
                for r in self.resource.values():
                    res.update(r)
            return 200, {}, {"resource": res}
        ...
        for r in roles:
            if r in self.resource:
                if name:
                    for _r in self.resource[r].keys():
                        if _r.split(":", maxsplit=1)[0] in name:
                            res[_r] = self.resource[r][_r]
                else:
                    res.update(self.resource[r])
